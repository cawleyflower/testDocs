...
pageID: 8362442
aliases:
 - /old/Cumulus_Linux_36/Address_Resolution_Protocol_-_ARP.html
...
<p>The <code>arp_filter</code> is primarily used when multiple interfaces reside in the same subnet and is used to allow/disallow which interfaces respond to ARP requests. In the case of <a href="/old/Cumulus_Linux_36/Open_Shortest_Path_First_-_OSPF_-_Protocol.html">OSPF</a> using IP unnumbered interfaces, many interfaces appear to be in the same subnet, and so actually contain the same address. If multiple interfaces are used between a pair of routers, having <code>arp_filter</code> set to 1 causes forwarding to fail.</p>
...
[VRR](/old/Cumulus_Linux_36/Virtual_Router_Redundancy_-_VRR.html), as it
creates a "-v0" interface in the same broadcast domain, make sure to use
