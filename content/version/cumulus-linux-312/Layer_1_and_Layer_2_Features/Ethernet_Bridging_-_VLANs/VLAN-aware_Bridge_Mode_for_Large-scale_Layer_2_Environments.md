...
weight: 273
aliases:
 - /display/CL31/VLAN-aware+Bridge+Mode+for+Large-scale+Layer+2+Environments
...
[Spanning
Tree](/version/cumulus-linux-312/Layer_1_and_Layer_2_Features/Spanning_Tree_and_Rapid_Spanning_Tree).
...
## <span id="src-5122017_VLAN-awareBridgeModeforLarge-scaleLayer2Environments-svi" class="confluence-anchor-link"></span><span>VLAN Layer 3 Addressing - Switch Virtual Interfaces and other VLAN Attributes </span>
...
## <span id="src-5122017_VLAN-awareBridgeModeforLarge-scaleLayer2Environments-glob" class="confluence-anchor-link"></span><span>Using the glob Keyword to Configure Multiple Ports in a Range</span>
...
added to a single VLAN-aware bridge. A 1:1 association is used between
the VXLAN VNI and the VLAN, using the bridge0access VLAN definition on
the VXLAN, and the VLAN membership definition on the local bridge member
interfaces.

The configuration examples below show the differences between a VXLAN
configured for traditional bridge mode and one configured for VLAN-aware
mode.

{{%notice note%}}

The current tested scale limit for Cumulus Linux 3.1 is 512 VNIs.

{{%/notice%}}

The following configuration uses head end replication (HER) along with
...
The VLAN translation between different bridge member interfaces is not
supported in Cumulus Linux 3.1. If VLAN translation is required,
configure the VXLANs using traditional bridge mode.
...
## <span>Caveats and Errata</span>
  ...
  - **<span id="src-5122017_VLAN-awareBridgeModeforLarge-scaleLayer2Environments-range"></span>Reserved
    ...
    2.  [Restart
        ...
        forwarding will be
        [interrupted](Configuring_switchd.html#src-5121932_Configuringswitchd-restartswitchd).
