...
Cumulus RMP and right before and after you commit a switch configuration
using
...
The primary snapshot components are:
  ...
  - [btrfs](https://btrfs.wiki.kernel.org/index.php/Main_Page) — an
    underlying file system in Cumulus RMP, which supports snapshots.
...
If you're upgrading from a version of Cumulus RMP earlier than version
...
As described above, snapshots are taken automatically:
  ...
  - Before and after you update Cumulus RMP by running `apt-get
...
If you need to restore Cumulus RMP to an earlier state, you can roll
...
You can configure Cumulus RMP to take hourly snapshots. You need to
...
Cumulus RMP excludes a number of directories when it takes a snapshot of
