...
note](Monitoring_System_Hardware.html#src-7112351_MonitoringSystemHardware-smond)
for details.
...
ever edited (for example, through `vtysh` or `frr.conf`), the integrated
...
<td><p>The log file is configurable in FRRouting. When FRRouting first boots, it boots using the non-integrated config so each routing protocol has its own log file. After booting up, FRRouting switches over to using the integrated configuration which means that all logs go to a single place.</p>
<p>To edit where log files go use the command <code>log file &lt;location&gt;</code>. By default, FRRouting logs are not sent to syslog. This can be enabled using the command <code>log syslog &lt;level&gt;</code>. After this, logs go through rsyslog and into <code>/var/log/syslog</code>.</p></td>
