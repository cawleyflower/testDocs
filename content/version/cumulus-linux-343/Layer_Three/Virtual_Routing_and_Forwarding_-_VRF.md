...
characteristics:
  ...
  - Listen sockets used by services are VRF-global by default unless the
    ...
    VRF](Management_VRF.html#src-7112679_ManagementVRF-services).
...
interfaces. Keep in mind the following for a VRF table:
  ...
  - Names for VRF tables can be up to 15 characters. However, you
    **cannot** use the name *mgmt*, as this name can **only** be used
    for [management
    VRF](/version/cumulus-linux-343/Layer_Three/Management_VRF).
...
### <span>Configure Route Leaking</span>
...
### <span id="src-7112681_VirtualRoutingandForwarding-VRF-vrf_cmd" class="confluence-anchor-link"></span><span>Using the vrf Command</span>
...
#### <span id="src-7112681_VirtualRoutingandForwarding-VRF-exec" class="confluence-anchor-link"></span><span>Running IPv4 and IPv6 Commands in a VRF Context </span>
...
chapter](Management_VRF.html#src-7112679_ManagementVRF-services) details
how to do this.
...
[BFD](/version/cumulus-linux-343/Layer_Three/Bidirectional_Forwarding_Detection_-_BFD),
...
configurations](/version/cumulus-linux-343/Layer_Three/Border_Gateway_Protocol_-_BGP)
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="#src-7112681_VirtualRoutingandForwarding-VRF-exec">see above</a> for more details):</p>
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="#src-7112681_VirtualRoutingandForwarding-VRF-exe">see above</a> for more details):</p>
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="#src-7112681_VirtualRoutingandForwarding-VRF-ex">see above</a> for more details):</p>
...
## <span>Caveats and Errata</span>
  ...
  - You cannot configure [EVPN address
    families](/version/cumulus-linux-343/Network_Virtualization/Ethernet_Virtual_Private_Network_-_EVPN)
    within a VRF.
