...
weight: 119
aliases:
 - /display/CL332/Multi-Chassis+Link+Aggregation+-+MLAG
 - /pages/viewpage.action?pageId=5868990
pageID: 5868990
...
## <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-reqs" class="confluence-anchor-link"></span><span>MLAG Requirements</span>
...
MLAG has these requirements:
  ...
  - The dual-connected devices (servers or switches) can use LACP (IEEE
    ...
    [bond](/version/cumulus-linux-332/Layer_One_and_Two/Bonding_-_Link_Aggregation).
    ...
    [balance-xor
    mode](Bonding_-_Link_Aggregation.html#src-5868970_Bonding-LinkAggregation-balance_xor)
...
## <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-configuring" class="confluence-anchor-link"></span><span>Configuring MLAG</span>
...
configuration changes, including:
  ...
  - Port configuration: For example, VLAN membership,
    [MTU](#src-5868990_Multi-ChassisLinkAggregation-MLAG-mtu) and
    bonding parameters.
...
[below](#src-5868990_Multi-ChassisLinkAggregation-MLAG-example).
...
[See below](#src-5868990_Multi-ChassisLinkAggregation-MLAG-backup) for
...
unnumbered](/version/cumulus-linux-332/Layer_Three/Border_Gateway_Protocol_-_BGP)
interfaces.

{{%/notice%}}

### <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-roles" class="confluence-anchor-link"></span><span>Understanding Switch Roles and Setting Priority</span>
...
## <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-example" class="confluence-anchor-link"></span><span>Example MLAG Configuration</span>
...
use](VLAN-aware_Bridge_Mode_for_Large-scale_Layer_2_Environments.html#src-5868982_VLAN-awareBridgeModeforLarge-scaleLayer2Environments-range).
...
net add clag peer sys-mac 44:38:39:FF:00:02 interface swp49-50 secondary backup-ip 192.168.1.11
net add clag port bond server1 interface swp1 clag-id 1
net add clag port bond server2 interface swp2 clag-id 2
net add bond server1-2 bridge access 100
net add bond server1-2 stp portadminedge
...
[untagged/native](/version/cumulus-linux-332/Layer_One_and_Two/Ethernet_Bridging_-_VLANs/Traditional_Mode_Bridges)
...
## <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-protodown" class="confluence-anchor-link"></span><span>Peer Link Interfaces and the protodown State</span>
...
### <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-backup" class="confluence-anchor-link"></span><span>Specifying a Backup Link</span>
...
#### <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-vrf_backup" class="confluence-anchor-link"></span><span>Specifying a Backup Link to a VRF</span>

You can configure the backup link to a
[VRF](/version/cumulus-linux-332/Layer_Three/Virtual_Routing_and_Forwarding_-_VRF)
or [management
VRF](/version/cumulus-linux-332/Layer_Three/Management_VRF). Include the
...
protocol such as
[VRR](/version/cumulus-linux-332/Layer_One_and_Two/Virtual_Router_Redundancy_-_VRR/)
...
You cannot configure routing (either OSPF or BGP) over an MLAG bond, as
there is a 50% chance the traffic will arrive on the wrong switch and
get lost. This is because MLAG has two hashes that get created when you
route across the MLAG pair:

  - A layer 3 hash that identifies which OSPF or BGP neighbor the
    traffic is going to (either leaf01 or leaf02).

  - A layer 2 hash on the bond. Half of the time, the layer 2 hash could
    cause the traffic to end up on the leaf opposite to the layer 3
    hash.

The traffic is dropped on the peerlink because it is viewed as a loop;
traffic should not arrive on the peerlink from dual-connected hosts.
...
MLAG](#src-5868990_Multi-ChassisLinkAggregation-MLAG-configuring)
section.

### <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-mtu" class="confluence-anchor-link"></span><span>Understanding MTU in an MLAG Configuration</span>

Note that the
[MTU](/version/cumulus-linux-332/Interface_Configuration_and_Management/Layer_1_and_Switch_Port_Attributes)
...
`mtu 9216`](/version/cumulus-linux-332/Interface_Configuration_and_Management/Layer_1_and_Switch_Port_Attributes)
...
Assurance](Spanning_Tree_and_Rapid_Spanning_Tree.html#src-5869002_SpanningTreeandRapidSpanningTree-bpdu).)
...
### <span id="src-5868990_Multi-ChassisLinkAggregation-MLAG-drops" class="confluence-anchor-link"></span><span>Large Packet Drops on the Peerlink Interface</span>
