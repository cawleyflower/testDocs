...
To remove the plugins, [see
below](/old/Cumulus_Linux_35/#src-8357341_RADIUSAAA-remove).
...
VRF](/old/Cumulus_Linux_35/Management_VRF.html). If you are specifying
the `src_ip` field for a server entry, and that interface is in a VRF,
you may also need to set it. At this time, you cannot specify more than
one VRF. The example below shows the vrf setting in the
`/etc/pam_radius_auth.conf` file.
...
## Related Information
  ...
  - [TACACS+ client](/old/Cumulus_Linux_35/TACACS_Plus.html)
