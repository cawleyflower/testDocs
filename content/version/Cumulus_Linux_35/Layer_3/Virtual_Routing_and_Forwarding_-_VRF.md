...
pageID: 8357738
aliases:
 - /old/Cumulus_Linux_35/Virtual_Routing_and_Forwarding_-_VRF.html
...
characteristics:
  ...
  - Applications can use existing interfaces to operate in a VRF context
    ...
    VRF](/old/Cumulus_Linux_35/Management_VRF.html) is enabled, logins
    to the switch default to the management VRF. This is a convenience
    for users to not have to specify management VRF for each command.
  ...
  - Listen sockets used by services are VRF-global by default unless the
    ...
    VRF](/old/Cumulus_Linux_35/Management_VRF.html#src-8357736_ManagementVRF-services).
...
interfaces. Keep in mind the following for a VRF table:
  ...
  - Names for VRF tables can be up to 15 characters. However, you
    **cannot** use the name *mgmt*, as this name can **only** be used
    for [management VRF](/old/Cumulus_Linux_35/Management_VRF.html).
...
chapter](/old/Cumulus_Linux_35/Management_VRF.html#src-8357736_ManagementVRF-services)
...
[OSPFv2](/old/Cumulus_Linux_35/Open_Shortest_Path_First_-_OSPF_-_Protocol.html)
and [static routing](/old/Cumulus_Linux_35/Routing.html) (IPv4 and IPv6)
...
[FRRouting](/old/Cumulus_Linux_35/FRRouting_Overview.html) learns of
...
FRRouting too, but they become active only when configured with NCLU.
  ...
  - Static routes (IPv4 and IPv6) can be provisioned in a VRF by
    ...
    above](/old/Cumulus_Linux_35/#src-8357738_VirtualRoutingandForwarding-VRF-leakstatic).
...
configurations](/old/Cumulus_Linux_35/Border_Gateway_Protocol_-_BGP.html)
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="/old/Cumulus_Linux_35/#src-8357738_VirtualRoutingandForwarding-VRF-exec">see above</a> for more details):</p>
<pre><code>                   
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="/old/Cumulus_Linux_35/#src-8357738_VirtualRoutingandForwarding-VRF-exe">see above</a> for more details):</p>
<pre><code>                   
...
<p>{{%notice tip%}}</p>
<p>You can create this configuration using the <code>vrf</code> command (<a href="/old/Cumulus_Linux_35/#src-8357738_VirtualRoutingandForwarding-VRF-ex">see above</a> for more details):</p>
<pre><code>                   
...
## Caveats and Errata
  ...
  - You cannot configure [EVPN address
    families](/old/Cumulus_Linux_35/Ethernet_Virtual_Private_Network_-_EVPN.html)
    within a VRF.
