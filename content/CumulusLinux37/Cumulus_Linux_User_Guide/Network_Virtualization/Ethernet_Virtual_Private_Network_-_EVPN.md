...
features include:
  ...
  - Support for dual-attached hosts via [VXLAN active-active
    mode](VXLAN_Active-Active_Mode.html). MAC synchronization between
    the peer switches is done using
...
about FRR, refer to the [FRR Overview](FRRouting_Overview.html).

{{%/notice%}}

For information about VXLAN routing, including platform and hardware
limitations, see [VXLAN Routing](VXLAN_Routing.html).
...
needed in [FRRouting](FRRouting_Overview.html). Additional configuration
...
[OSPF](Open_Shortest_Path_First_-_OSPF.html) or static route underlay if
needed. This is a more complex configuration than using eBGP. In this
case, iBGP advertises EVPN routes directly between VTEPs, and the spines
...
For a bridge in [traditional mode](Traditional_Bridge_Mode.html), you
...
[NCLU](Network_Command_Line_Utility_-_NCLU.html). Here is an example
...
active-active mode](VXLAN_Active-Active_Mode.html). Both switches in the
[MLAG](Multi-Chassis_Link_Aggregation_-_MLAG.html) pair establish EVPN
...
Mode](VXLAN_Active-Active_Mode.html) chapter.
...
The following caveats apply to EVPN in this version of Cumulus Linux:
  ...
  - In a [VXLAN active-active](VXLAN_Active-Active_Mode.html)
