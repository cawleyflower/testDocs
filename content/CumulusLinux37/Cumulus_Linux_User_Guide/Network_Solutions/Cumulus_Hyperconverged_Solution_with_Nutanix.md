...
In addition, you can augment the deployment with:
  ...
  - [Cumulus on a
    ...
    touch provisioning](Zero_Touch_Provisioning_-_ZTP.html) Nutanix and
...
Cumulus HCS has two major components:
  ...
  - **Nutanix Webhook VLAN Provisioner**. Cumulus Linux switches
    ...
    bridge](VLAN-aware_Bridge_Mode.html), the MLAG peer link and the
...
## Prerequisites
  ...
  - [MLAG](Multi-Chassis_Link_Aggregation_-_MLAG.html) enabled on the
...
typical Nutanix deployment with the following configuration:
  ...
  - The eth0 management interface is configured for [management
    VRF](Management_VRF.html) via DHCP
...
[LLDP](Link_Layer_Discovery_Protocol.html) messages may not be getting
