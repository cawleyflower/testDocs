...
with anycast traffic, the SVI IP address is not unique to each rack;
<span style="color: #000000;"> it is typically shared amongst all racks.
Most EVPN ToR deployments only possess a single unique IP address, which
is the loopback IP address. </span>
