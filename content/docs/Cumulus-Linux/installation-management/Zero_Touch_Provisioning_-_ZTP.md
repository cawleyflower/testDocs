...
[CFEngine](https://cfengine.com) or possibly a custom, proprietary tool.
...
-   [Zero Touch Provisioning Using a Local
    File](#ZeroTouchProvisioning-ZTP-ZeroTouchProvisioningUsingaLocalFile)
-   [Zero Touch Provisioning Using a USB Drive
    (ZTP-USB)](#ZeroTouchProvisioning-ZTP-ZeroTouchProvisioningUsingaUSBDrive(ZTP-USB))
-   [Zero Touch Provisioning over
    DHCP](#ZeroTouchProvisioning-ZTP-ZeroTouchProvisioningoverDHCP)
    -   [Trigger ZTP over
        DHCP](#ZeroTouchProvisioning-ZTP-TriggerZTPoverDHCP)
    -   [Configure the DHCP
        Server](#ZeroTouchProvisioning-ZTP-ConfiguretheDHCPServer)
    -   [Inspect HTTP
        Headers](#ZeroTouchProvisioning-ZTP-http_headersInspectHTTPHeaders)
-   [Write ZTP Scripts ](#ZeroTouchProvisioning-ZTP-WriteZTPScripts)
-   [Best Practices for ZTP
    Scripts](#ZeroTouchProvisioning-ZTP-BestPracticesforZTPScripts)
    -   [Install a License](#ZeroTouchProvisioning-ZTP-InstallaLicense)
    -   [Test DNS
        Name Resolution](#ZeroTouchProvisioning-ZTP-TestDNSNameResolution)
    -   [Check the Cumulus Linux
        Release](#ZeroTouchProvisioning-ZTP-ChecktheCumulusLinuxRelease)
    -   [Apply Management VRF
        Configuration](#ZeroTouchProvisioning-ZTP-ApplyManagementVRFConfiguration)
    -   [Perform Ansible Provisioning
        Callbacks](#ZeroTouchProvisioning-ZTP-PerformAnsibleProvisioningCallbacks)
    -   [Disable the DHCP Hostname Override
        Setting](#ZeroTouchProvisioning-ZTP-DisabletheDHCPHostnameOverrideSetting)
    -   [NCLU in ZTP
        Scripts ](#ZeroTouchProvisioning-ZTP-NCLUinZTPScripts)
-   [Test ZTP Scripts](#ZeroTouchProvisioning-ZTP-TestZTPScripts)
-   [Common ZTP Script
    Errors](#ZeroTouchProvisioning-ZTP-CommonZTPScriptErrors)
-   [Manually Use the ztp
    Command](#ZeroTouchProvisioning-ZTP-ManuallyUsetheztpCommand)
-   [Notes](#ZeroTouchProvisioning-ZTP-Notes)
...
[ONIE](http://onie.org)-style waterfall
in `/var/lib/cumulus/ztp`, looking for the most specific name first, and
ending at the most generic:
...
You can also trigger the ZTP process manually by running
the `ztp --run <URL>` command, where the URL is the path to the ZTP
script.
...
2.  The `ztp` process searches the root filesystem of the newly mounted
    ...
    waterfall](https://github.com/opencomputeproject/onie/wiki/Quick-Start-Guide#directly-connected-scenario) (see
    the patterns and examples above), looking for the most specific name
    first, and ending at the most generic.
3.  The contents of the script are parsed to ensure it contains
    the `CUMULUS-AUTOPROVISIONING` flag (see [example
    scripts](#ZeroTouchProvisioning-ZTP-example_scripts)).
...
4.  The zero touch provisioning process requests the contents of the
    ...
    headers](#ZeroTouchProvisioning-ZTP-http_headers) containing details
    about the switch.
5.  The contents of the script are parsed to ensure it contains
    the `CUMULUS-AUTOPROVISIONING` flag (see [example
    scripts](#ZeroTouchProvisioning-ZTP-example_scripts)).
...
Additionally, you can specify the hostname of the switch with
...
The following script installs Cumulus Linux and its license from a
USB drive and applies a configuration: 
...
DNS names are frequently used in ZTP scripts.
The `ping_until_reachable` function tests that each DNS name resolves
into a reachable IP address. Call this function with each DNS target
used in your script before you use the DNS name elsewhere in your
script. 

The following example shows how to call
the `ping_until_reachable` function in the context of a larger task.
...
`/usr/bin/vrf task exec mgmt`; for example,
`/usr/bin/vrf task exec mgmt apt-get update -y`.
...
Use the translate (`tr`) command on any Linux system to remove
the `'\r'` characters from the file.
