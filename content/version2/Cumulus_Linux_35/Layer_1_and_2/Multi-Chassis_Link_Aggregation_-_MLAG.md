...
slug: Multi-Chassis_Link_Aggregation_-_MLAG
pageID: 8357455
...
device should be placed into a [bond](/Bonding_-_Link_Aggregation.html),
...
unnumbered](/Border_Gateway_Protocol_-_BGP.html) interfaces.
...
mode](/Traditional_Mode_Bridges.html) instead of [VLAN-aware
mode](/VLAN-aware_Bridge_Mode_for_Large-scale_Layer_2_Environments.html).

{{%notice note%}}

You cannot configure a traditional mode bridge using
[NCLU](/Network_Command_Line_Utility_-_NCLU.html); you must configure it
manually in the `/etc/network/interfaces` file.
...
[untagged/native](/Traditional_Mode_Bridges.html) ports on a bridge
(note the absence of any VLANs in the `bridge-ports` line and the lack
of the `bridge-vlan-aware` parameter below):
...
VRF](/Management_VRF.html). Include the name of the VRF or management
VRF when you specify `clagd-backup-ip <ADDRESS> vrf <VRF name>`. Here is
...
protocol such as [VRR](/Virtual_Router_Redundancy_-_VRR.html) (Virtual
Router Redundancy) between the spine switch pair to support
...
[OSPF](/Open_Shortest_Path_First_-_OSPF_-_Protocol.html) or
...
[IGMP snooping](/IGMP_and_MLD_Snooping.html) processes IGMP reports
received on a bridge port in a bridge to identify hosts that are
configured to receive multicast traffic destined to that group. An IGMP
query message received on a port is used to identify the port that is
connected to a router and configured to receive multicast traffic.
...
Note that the [MTU](/Layer_1_and_Switch_Port_Attributes.html) in MLAG
traffic is determined by the bridge MTU. Bridge MTU is determined by the
lowest MTU setting of an interface that is a member of the bridge. If an
MTU other than the default of 1500 bytes is desired, you must configure
the MTU on each physical interface and bond interface that are members
of the MLAG bridges in the entire bridged domain.
...
`mtu 9216`](/Layer_1_and_Switch_Port_Attributes.html) for each of the
following bond interfaces, since they are members of bridge *bridge*:
