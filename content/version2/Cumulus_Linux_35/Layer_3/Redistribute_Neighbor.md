...
weight: 183
aliases:
 - /display/CL35/Redistribute+Neighbor
 - /pages/viewpage.action?pageId=8357751
slug: Redistribute_Neighbor
pageID: 8357751
...
Linux relies on
[ECMP](/Equal_Cost_Multipath_Load_Sharing_-_Hardware_ECMP.html) to load
...
Additionally, install and use [ifplugd](/ifplugd.html). `ifplugd`
modifies the behavior of the Linux routing table when an interface
undergoes a link transition (carrier up/down). The Linux kernel by
default leaves routes up even when the physical interface is unavailable
(NO-CARRIER).
