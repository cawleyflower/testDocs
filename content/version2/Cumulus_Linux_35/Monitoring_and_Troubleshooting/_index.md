...
slug: Monitoring_and_Troubleshooting
pageID: 8357374
...
| 20-clagd.conf     | Logs `clagd` messages to `/var/log/clagd.log` for [MLAG](/Multi-Chassis_Link_Aggregation_-_MLAG.html).                      |
...
| 30-ptmd.conf      | Logs `ptmd` messages to `/var/log/ptmd.log` for [Prescription Topology Manager](/Prescriptive_Topology_Manager_-_PTM.html). |
| 35-rdnbrd.conf    | Logs `rdnbrd` messages to `/var/log/rdnbrd.log` for [redistribute neighbor](/Redistribute_Neighbor.html).                   |
| 40-netd.conf      | Logs `netd` messages to `/var/log/netd.log` for [NCLU](/Network_Command_Line_Utility_-_NCLU.html).                          |
...
You can write to syslog with [management VRF](/Management_VRF.html)
enabled by applying the following configuration; this configuration is
commented out in the `/etc/rsyslog.d/11-remotesyslog.conf` file:
