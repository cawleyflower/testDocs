...
[FRRouting](/FRRouting_Overview.html). To enable SNMP support for
FRRouting, you need to:
...
<tr class="odd">
<td><p><strong><a href="/Using_Nutanix_Prism_as_a_Monitoring_Tool.html">Nutanix Prism CLI</a></strong> (SSH to the cluster IP address)</p></td>
...
`bgp4_pp.py` and `cl_poe_pp.py` scripts:
  ...
  - `bgp4_pp.py` is now handled by [FRRouting](/FRRouting_Overview.html)
  ...
  - `cl_poe_pp.py` is disabled by default as only some platforms that
    ...
    Ethernet](/Power_over_Ethernet_-_PoE.html).
