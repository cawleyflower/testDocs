...
slug: Network_Command_Line_Utility_-_NCLU
pageID: 8357362
...
using:
  ...
  - `net commit permanent`: Retains the
    [snapshot](/Using_Snapshots.html) taken when committing the change.
    Otherwise, the snapshots created from NCLU commands are cleaned up
    periodically via a snapper cron job.
  ...
  - `net del all`: Deletes all configurations and stops the IEEE 802.1X
    ...
    This command does not remove [management VRF](/Management_VRF.html)
...
`tacacs3` or `radius_user`, not the [TACACS](/TACACS_Plus.html) or
[RADIUS](/RADIUS_AAA.html) account name.
