...
commands](/Using_sudo_to_Delegate_Privileges.html), so be sure to
include them in the *sudo* group.

To access the switch without any password requires [booting into a
single shell/user mode](/Single_User_Mode_-_Boot_Recovery.html).
...
User Accounts](/Network_Command_Line_Utility_-_NCLU.html).
