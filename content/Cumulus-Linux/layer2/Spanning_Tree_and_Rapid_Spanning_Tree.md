...
-   [Supported Modes](#SpanningTreeandRapidSpanningTree-SupportedModes)
    -   [STP for a VLAN-aware
        Bridge](#SpanningTreeandRapidSpanningTree-STPforaVLAN-awareBridge)
    -   [STP within a Traditional Mode
        Bridge](#SpanningTreeandRapidSpanningTree-STPwithinaTraditionalModeBridge)
-   [View Bridge and STP Status and
    Logs](#SpanningTreeandRapidSpanningTree-ViewBridgeandSTPStatusandLogs)
-   [Customize Spanning Tree
    Protocol](#SpanningTreeandRapidSpanningTree-CustomizeSpanningTreeProtocol)
    -   [Spanning Tree
        Priority](#SpanningTreeandRapidSpanningTree-SpanningTreePriority)
    -   [PortAdminEdge (PortFast
        Mode)](#SpanningTreeandRapidSpanningTree-PortAdminEdge(PortFastMode))
    -   [PortAutoEdge](#SpanningTreeandRapidSpanningTree-PortAutoEdge)
    -   [BPDU Guard](#SpanningTreeandRapidSpanningTree-bpduBPDUGuard)
    -   [Bridge
        Assurance](#SpanningTreeandRapidSpanningTree-BridgeAssurance)
    -   [BPDU Filter](#SpanningTreeandRapidSpanningTree-BPDUFilter)
    -   [Storm Control](#SpanningTreeandRapidSpanningTree-StormControl)
    -   [Spanning Tree Parameter
        List](#SpanningTreeandRapidSpanningTree-paramsSpanningTreeParameterList)
    -   [Caveats and
        Errata](#SpanningTreeandRapidSpanningTree-CaveatsandErrata)
-   [Related
    Information](#SpanningTreeandRapidSpanningTree-RelatedInformation)

## Supported Modes

The STP modes Cumulus Linux supports vary depending upon whether
the traditional or VLAN-aware bridge driver mode is in use.

-   Bridges configured in* [VLAN-aware](VLAN-aware_Bridge_Mode) *mode
    operate **only** in RSTP mode. 
-   Bridges configured in [*traditional*
    mode](Traditional_Bridge_Mode) operate in both PVST and PVRST mode.
    The default is set to PVRST. Each traditional bridge has its own
    separate STP instance.

### STP for a VLAN-aware Bridge

VLAN-aware* *bridges only operate in RSTP mode. STP bridge protocol data
units (BPDUs) are transmitted on the native VLAN.
...
To check STP status for a bridge, run
the `net show bridge spanning-tree` command:
...
![](images/icons/grey_arrow_down.png){.expand-control-image}Using Linux
to check STP status ...
...
accompanied by the [BPDU guard](#SpanningTreeandRapidSpanningTree-bpdu)
feature.
...
For a bridge in [traditional mode](Ethernet_Bridging_-_VLANs),
configure `PortAdminEdge` under the bridge stanza
...
*PortAutoEdge* is an enhancement to the
...
transition* to/from the edge state of a port in a bridge. 
...
To re-enable PortAutoEdge for an interface, run
the the `net del interface <port> stp portautoedge no` command. The
following example re-enables PortAutoEdge on swp1: 
...
with `sudo ifup ` `[port]`, as shown in the example below.
...
[configuring `switchd`](Configuring_switchd). For example, to enable
...
Spanning tree parameters are defined in the
IEEE [802.1D](http://standards.ieee.org/findstds/standard/802.1D-2004.html), [802.1Q](http://standards.ieee.org/findstds/standard/802.1Q-2018.html) specifications.
...
between `mstpctl` and other vendors,[ read this knowledge base
...
file](https://docs.cumulusnetworks.com/display/DRAFT40/Network+Command+Line+Utility+-+NCLU#NetworkCommandLineUtility-NCLU-conf)
