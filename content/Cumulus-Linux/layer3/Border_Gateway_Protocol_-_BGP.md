...
-   [Autonomous System Number
    (ASN)](#BorderGatewayProtocol-BGP-AutonomousSystemNumber(ASN))
-   [eBGP and iBGP](#BorderGatewayProtocol-BGP-eBGPandiBGP)
-   [Route Reflectors](#BorderGatewayProtocol-BGP-RouteReflectors)
-   [ECMP with BGP](#BorderGatewayProtocol-BGP-ECMPecmpwithBGP)
    -   [Maximum Paths](#BorderGatewayProtocol-BGP-MaximumPaths)
    -   [BGP for Both IPv4 and
        IPv6](#BorderGatewayProtocol-BGP-BGPforBothIPv4andIPv6)
-   [Configure BGP](#BorderGatewayProtocol-BGP-config_bgpConfigureBGP)
-   [BGP Unnumbered
    Interfaces](#BorderGatewayProtocol-BGP-unnumberedBGPUnnumberedInterfaces)
    -   [BGP and Extended Next Hop
        Encoding](#BorderGatewayProtocol-BGP-BGPandExtendedNextHopEncoding)
    -   [Configure BGP Unnumbered
        Interfaces](#BorderGatewayProtocol-BGP-ConfigureBGPUnnumberedInterfaces)
    -   [Manage Unnumbered
        Interfaces](#BorderGatewayProtocol-BGP-ManageUnnumberedInterfaces)
    -   [How traceroute Interacts with BGP Unnumbered
        Interfaces](#BorderGatewayProtocol-BGP-HowtracerouteInteractswithBGPUnnumberedInterfaces)
    -   [Advanced: How Next Hop Fields Are
        Set](#BorderGatewayProtocol-BGP-Advanced:HowNextHopFieldsAreSet)
    -   [Limitations](#BorderGatewayProtocol-BGP-Limitations)
-   [RFC 5549 Support with Global IPv6 Peers (Cumulus Linux 3.7.2 and
    later)](#BorderGatewayProtocol-BGP-rfc-5549RFC5549SupportwithGlobalIPv6Peers(CumulusLinux3.7.2andlater))
    -   [Configure RFC 5549 Support with Global IPv6
        Peers  ](#BorderGatewayProtocol-BGP-ConfigureRFC5549SupportwithGlobalIPv6Peers)
    -   [Show IPv4 Prefixes Learned with IPv6 Next
        Hops](#BorderGatewayProtocol-BGP-ShowIPv4PrefixesLearnedwithIPv6NextHops)
-   [BGP add-path](#BorderGatewayProtocol-BGP-add-pathBGPadd-path)
    -   [BGP add-path RX](#BorderGatewayProtocol-BGP-BGPadd-pathRX)
    -   [BGP add-path TX](#BorderGatewayProtocol-BGP-BGPadd-pathTX)
-   [Fast Convergence Design
    Considerations](#BorderGatewayProtocol-BGP-FastConvergenceDesignConsiderations)
-   [Peer Groups to Simplify
    Configuration](#BorderGatewayProtocol-BGP-peergroupsPeerGroupstoSimplifyConfiguration)
-   [Configure BGP Dynamic
    Neighbors](#BorderGatewayProtocol-BGP-ConfigureBGPDynamicNeighbors)
-   [Configure BGP Peering Relationships across
    Switches](#BorderGatewayProtocol-BGP-ConfigureBGPPeeringRelationshipsacrossSwitches)
-   [Configure MD5-enabled BGP
    Neighbors](#BorderGatewayProtocol-BGP-ConfigureMD5-enabledBGPNeighbors)
-   [Configure eBGP
    Multihop](#BorderGatewayProtocol-BGP-ConfigureeBGPMultihop)
-   [Configure BGP TTL
    Security](#BorderGatewayProtocol-BGP-ConfigureBGPTTLSecurity)
-   [Configure Graceful BGP
    Shutdown](#BorderGatewayProtocol-BGP-ConfigureGracefulBGPShutdown)
-   [Configuration Tips](#BorderGatewayProtocol-BGP-ConfigurationTips)
    -   [BGP Advertisement Best
        Practices](#BorderGatewayProtocol-BGP-BGPAdvertisementBestPractices)
    -   [Multiple Routing Tables and
        Forwarding](#BorderGatewayProtocol-BGP-MultipleRoutingTablesandForwarding)
    -   [BGP Community
        Lists](#BorderGatewayProtocol-BGP-BGPCommunityLists)
    -   [Additional Default
        Settings](#BorderGatewayProtocol-BGP-AdditionalDefaultSettings)
    -   [Configure BGP Neighbor
        maximum-prefixes](#BorderGatewayProtocol-BGP-ConfigureBGPNeighbormaximum-prefixes)
-   [Troubleshooting](#BorderGatewayProtocol-BGP-Troubleshooting)
    -   [Log Neighbor State
        Changes](#BorderGatewayProtocol-BGP-LogNeighborStateChanges)
    -   [Troubleshoot Link-local
        Addresses](#BorderGatewayProtocol-BGP-TroubleshootLink-localAddresses)
-   [Enable Read-only
    Mode](#BorderGatewayProtocol-BGP-EnableRead-onlyMode)
-   [Apply a Route Map for Route
    Updates](#BorderGatewayProtocol-BGP-ApplyaRouteMapforRouteUpdates)
    -   [Filter Routes from BGP into
        Zebra](#BorderGatewayProtocol-BGP-FilterRoutesfromBGPintoZebra)
    -   [Filter Routes from Zebra into the Linux
        Kernel](#BorderGatewayProtocol-BGP-FilterRoutesfromZebraintotheLinuxKernel)
-   [Protocol Tuning](#BorderGatewayProtocol-BGP-ProtocolTuning)
    -   [Converge Quickly On Link
        Failures](#BorderGatewayProtocol-BGP-ConvergeQuicklyOnLinkFailures)
    -   [Converge Quickly On Soft
        Failures](#BorderGatewayProtocol-BGP-ConvergeQuicklyOnSoftFailures)
    -   [Reconnect Quickly](#BorderGatewayProtocol-BGP-ReconnectQuickly)
    -   [Advertisement
        Interval](#BorderGatewayProtocol-BGP-AdvertisementInterval)
-   [Caveats and Errata](#BorderGatewayProtocol-BGP-CaveatsandErrata)
    -   [ttl-security
        Issue](#BorderGatewayProtocol-BGP-ttl-securityIssue)
    -   [BGP Dynamic Capabilities not
        Supported](#BorderGatewayProtocol-BGP-BGPDynamicCapabilitiesnotSupported)
    -   [Related
        Information](#BorderGatewayProtocol-BGP-RelatedInformation)

## Autonomous System Number (ASN)

One of the key concepts in BGP is an *autonomous system number* or ASN.
An [autonomous
system](https://en.wikipedia.org/wiki/Autonomous_System_%28Internet%29) is
...
![](attachments/8362926/8362927.png){width="300"}
...
after the `net add bgp neighbor <IPV4/IPV6> activate` command;
otherwise, the `route-reflector-client` command is ignored. For example:
...
To configure a cluster ID on a route reflector, run
the `net add bgp cluster-id (<ipv4>|<1-4294967295>)` command. You can
...
To perform ECMP in BGP, you may need to
configure `net add bgp bestpath as-path multipath-relax` (if you are
using eBGP).
...
Unlike OSPF, which has separate versions of the protocol to
...
families over a single IPv4 session or over a single IPv6 session.
...
1.  Enable the BGP and Zebra daemons (`zebra` and `bgpd`), then enable
    ...
    FRRouting](Configuring_FRRouting).
...
3.  Specify where to disseminate routing information:
    ...
    the `activate` command. You must specify the `activate` command for
    each address family that is being announced by the BGP session.
...
encoding* (ENHE), which is defined by [RFC
5549](https://tools.ietf.org/html/rfc5549). BGP unnumbered interfaces
...
Peers](#BorderGatewayProtocol-BGP-rfc-5549) below.
...
Support with Global IPv6 Peers](#BorderGatewayProtocol-BGP-rfc-5549)).
...
group](#BorderGatewayProtocol-BGP-peergroups) (making sure to substitute
for the interface and peer group below):
...
The following commands show how the IPv4 link-local
...
prefix with IPv6 next hop in the kernel:
...
Every router or end host must have an IPv4 address to complete
...
-   Whether peering to a global IPv6 address or link-local IPv6 address,
    ...
    2.  Otherwise (if it is not reflecting the route), two next hops are
        ...
        is *directly connected*) and the route is either a
...
-   `route-map` can impose two next hops in scenarios where Cumulus
    ...
    specifying `set ipv6 nexthop link-local`.
...
-   Whenever two next hops are being sent, the link-local next hop (the
    ...
    to `nh-local-unchanged` or `route-map` has set the link-local next
    hop.
...
draft [draft-kato-bgp-ipv6-link-local-00.txt](https://tools.ietf.org/html/draft-kato-bgp-ipv6-link-local-00), "BGP4+
...
unicast and link-local next hop addresses, with either *unnumbered*
or *numbered* BGP.
...
The above commands create the following configuration in
the `/etc/frr/frr.conf` file:
...
To show IPv4 prefixes learned with IPv6 next hops, you can
run `net show bgp ipv4 unicast` commands.
...
existing capabilities are listed in the subsection *Add Path*,
below *Neighbor capabilities:*
...
The `net show bgp 1.1.1.1/32` from r7 has "bestpath-from-AS 100" so the
user can see what the bestpath is from each AS:
...
other `neighbor` command options remain the same.
...
Consider the following example configuration in
the `/etc/frr/frr.conf` file:
...
(`net add interface <interface> ipv6 nd ra-interval <interval>`) to
...
`interval` is measured in seconds and defaults to 10 seconds.
...
the `activate` and `next-hop-self` commands for even 60 neighbors makes
...
You configure dynamic neighbors using
the `bgp listen range <IP address> peer-group <GROUP>` command. After
...
To limit the number of dynamic peers, specify the limit in
...
A BGP peering relationship is typically initiated with
the `neighbor x.x.x.x remote-as [internal|external]` command.
...
You can make this distinction using the `neighbor` command or
the `peer-group` command. 
...
3.  Confirm the configuration has been implemented with
    the `net show bgp summary` command:
...
6.  Confirm the configuration has been implemented with
    the `net show bgp summary` command:
...
2.  Confirm the configuration with
    the `net show bgp neighbor <ip>` command:
...
To configure graceful BGP shutdown for the current node, run
the `net add bgp graceful-shutdown` command:
...
To disable graceful shutdown for the current node, run
the `net del bgp graceful-shutdown` command:
...
![](attachments/8362926/8362925.png){height="400"}
...
using [management VRF](Management_VRF) (multiple routing tables and
forwarding).
...
lists* ](http://www.nongnu.org/quagga/docs/docs-multi/BGP-Community-Lists.html#BGP-Community-Lists) to
define a BGP community to tag one or more routes. You can then use the
communities to apply route policy on either egress or ingress.
...
To show a more detailed breakdown of a specific neighbor, run
the `net show bgp neighbor <neighbor>` command:
...
the `net show bgp <ip address/prefix>` command:
...
the `log-neighbor-changes` command, which is enabled by default.

The output is sent to the specified log file,
usually `/var/log/frr/bgpd.log`, and looks like this:
...
then `Neighbor address(s):` has the other end's link-local address. That
is the address that BGP uses when BGP is enabled on that interface.
...
To enable read-only mode, run
the `net add bgp update-delay <max-delay in 0-3600 seconds> [<establish-wait in 1-3600 seconds>]` command.   
The following example command enables read-only mode, sets
the `max-delay` timer to 300 seconds and the `establish-wait` timer to
90 seconds.
...
-   All the configured peers (except the shutdown peers) have sent an
    ...
    the `max-delay` timer starts until the `establish-wait` period
    ends. 
...
To show information about the state of the update delay, run
the `show bgp summary` command.

## Apply a Route Map for Route Updates

There are two ways you can apply [route
maps](http://www.nongnu.org/quagga/docs/docs-multi/Route-Map.html#Route-Map) for
BGP:
...
enabled for all
links: `link-detect` and `ttl-security hops <hops>`. `ttl-security hops` specifies
how many hops away the neighbor is. For example, in a Clos topology,
every peer is at most 1 hop away.
...
FRRouting `watchquagga` daemon, which monitors the various FRRouting
daemons, will attempt to restart it. If the process is also
hung, `watchquagga` will attempt to restart the process. BGP itself has
...
seconds. The following example shows how to change these timers:
...
of `advertisement-interval` seconds between sending consecutive updates
...
See this [IETF
draft](http://tools.ietf.org/html/draft-jakma-mrai-02) for more details
...
the `net add acl` command to explicitly add the relevant entry to
hardware.

For example, you can configure a file, such
as `/etc/cumulus/acl/policy.d/01control_plane_bgp.rules`, with a rule
like this for TTL:
...
-   [Bidirectional forwarding
    detection](Bidirectional_Forwarding_Detection_-_BFD) (BFD) and BGP
-   [Wikipedia entry for
...
[bgp\_route\_reflectors.png](attachments/8362926/8362927.png)
(image/png)  
![](images/icons/bullet_blue.gif){width="8" height="8"} [BGP
Advertisement Best Practices.png](attachments/8362926/8362925.png)
(image/png)  
