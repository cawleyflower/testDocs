...
Hardware datapath configuration manages packet buffering, queueing
and scheduling in hardware. There are two configuration input files:
...
-   `/usr/lib/python2.7/dist-packages/cumulus/__chip_config/[bcm|mlx]/datapath.conf`,
    ...
    support engineer](https://support.cumulusnetworks.com/hc/en-us/requests/new) to do so.

Each packet is assigned to an ASIC Class of Service (CoS) value based on
the packet's priority value stored in the 802.1p (Class of Service)
...
-   [Commands](#BufferandQueueManagement-Commands)
-   [Example Configuration
    File](#BufferandQueueManagement-ExampleConfigurationFile)
-   [Configure Traffic Marking through ACL
    Rules](#BufferandQueueManagement-ConfigureTrafficMarkingthroughACLRules)
-   [Configure Priority Flow
    Control](#BufferandQueueManagement-pfcConfigurePriorityFlowControl)
-   [Port Groups](#BufferandQueueManagement-PortGroups)
-   [Configure Link
    Pause](#BufferandQueueManagement-pauseConfigureLinkPause)
-   [Configure Cut-through Mode and Store and Forward
    Switching](#BufferandQueueManagement-cut_through_modeConfigureCut-throughModeandStoreandForwardSwitching)
-   [Configure Explicit Congestion
    Notification](#BufferandQueueManagement-ecnConfigureExplicitCongestionNotification)
-   [Check Interface Buffer
    Status](#BufferandQueueManagement-CheckInterfaceBufferStatus)
-   [Related Information](#BufferandQueueManagement-RelatedInformation)

## Commands

If you modify the configuration in the
`/etc/cumulus/datapath/traffic.conf` file, you must
[restart `switchd`](Configuring-switchd_8362561.html#Configuringswitchd-restartswitchd)
...
Spectrum [platforms](http://cumulusnetworks.com/hcl/) only. However, see
the note above for all the supported ASICs.
...
-   Specifying the name of the port group in `pfc.port_group_list` in
    ...
    \[pfc\_port\_group\]*.
-   Assigning a CoS value to the port group
    in `pfc.pfc_port_group.cos_list` setting. Note that
    *pfc\_port\_group* is the name of a port group you specified above
    and is used throughout the following settings.
-   Populating the port group with its member ports
    in `pfc.pfc_port_group.port_set`.
...
-   Enabling the egress port to receive notifications and act on
    them (`pfc.pfc_port_group.rx_enable`). The default is *true*.
...
in `/var/lib/cumulus/porttab`. The syntax supports:
...
[Restart `switchd` ](Configuring-switchd_8362561.html#Configuringswitchd-restartswitchd)to
allow the PFC configuration changes to take effect:
...
how you configure [priority flow control](Buffer_and_Queue_Management).
...
To disable link pause, comment out the `link_pause*` section
...
To enable store and forward switching,
set `cut_through_enable` to *false* in `/etc/cumulus/datapath/traffic.conf`:
...
*Explicit Congestion Notification* (ECN) is defined by [RFC
3168](https://tools.ietf.org/html/rfc3168). ECN gives a Cumulus Linux
...
ECN is supported on [Broadcom Tomahawk, Trident II, Trident II+ and
Trident3, and Mellanox Spectrum
switches](https://cumulusnetworks.com/hcl) only.

![](images/icons/grey_arrow_down.png){.expand-control-image}Click to
learn how to configure ECN ...
...
[Restart `switchd` ](Configuring-switchd_8362561.html#Configuringswitchd-restartswitchd)to allow
the ECN configuration changes to take effect:
...
monitoring chapter](ASIC_Monitoring) for details.
